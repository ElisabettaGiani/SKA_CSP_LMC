...
class CspMaster(with_metaclass(DeviceMeta,SKAMaster)):
        ...
        if evt.err is False:
            try:
                if ("state" == evt.attr_value.name) or ("State" == evt.attr_value.name): 
                    #print("time: {} New value: {}".format(time.time(), evt.attr_value.value))
                    print("Attribute {} new value: {}".format(evt.attr_name, evt.attr_value.value))
    ...
    def __set_csp_health_state(self):
        ...
        :return None
        """
        print("Call to set_csp_health_state")
    ...
    def init_device(self):
        ...
        # sub-element State and healthState
        ...
        # initialize the list with CSP sub-element FQDNs
        self._se_list = []
        # build the list with the CSP sub-element FQDN
        self._se_list.append(self.CspMidCbf)
        self._se_list.append(self.CspMidPss)
        self._se_list.append(self.CspMidPst)
        ...
        for nelem in range(0,len(self._se_list)):
            try: 
               ...
               #  print("exception:{}".format(df.args[err].reason))
